...
public:
   ...
   * This function will call helper function buildTree.
    ...
    */
   PNG render();
   Node * find(Node* root, const pair<int, int> &key);
   pair<int,int> findNewKey(pair<int,int> key, int k, int X, int Y);
   ...
   * You may want a recursive helper function for this one.
   ...
   bool canPrune(Node* croot, double tol, HSLAPixel avgPixel);

    /* returns the number of nodes in the current toqutree. primarily used
     * for debugging and testing.
     */
    int size();
    int size(const Node* croot);
...
private:
   ...
   */
   ...
   Node * copy(Node * croot, const Node * other);
